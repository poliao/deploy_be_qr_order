...
export const AddMenu = () => {
  ...
  const addSection = () => {
    setOptionsMenu((prevOptions) => [
      ...prevOptions,
      { option_name: "", optiondetail: [{ optiondetail: "" }] },
    ]);
  };

  const addOptionDetail = (index) => {
    const newOptionsMenu = [...optionsmenu];
    newOptionsMenu[index].optiondetail.push({ optiondetail: "" });
  ...
  const handleOptionDetailChange = (event, sectionIndex, detailIndex) => {
    const value = event.target.value;
    const newOptionsMenu = [...optionsmenu];
    newOptionsMenu[sectionIndex].optiondetail[detailIndex].optiondetail = value;
    setOptionsMenu(newOptionsMenu);
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const imageResponse = await fetch('http://localhost:8080/admin/menus/upload', {
        method: 'POST',
        body: imageFormData,
      });
